...
static void printFlow(u_int16_t id, struct ndpi_flow_info *flow, u_int16_t thread_id) {
  ...
  if(!json_flag) {
    ...
    */
    ...
    {
      char buf[64];
     // fprintf(out, "%s %s %s\n",flow->src_name,flow->dst_name,ndpi_protocol2name(ndpi_thread_info[thread_id].workflow->ndpi_struct,
		//		 flow->detected_protocol, buf, sizeof(buf)));
	  fprintf(out, "%s %s %s",flow->src_name,flow->dst_name,ndpi_protocol2name(ndpi_thread_info[thread_id].workflow->ndpi_struct,
				 flow->detected_protocol, buf, sizeof(buf)));
    }
	else
	{
...
#endif
/*********extract certain protocol***********/

void extractProtoAndSave(FILE* fp,char* msg)
{
  char proto[4096][100];
  char line[200];
  char ip[150];
  ...
  while(fgets(line,200,fp)!=NULL)
  {
    ...
    if(strstr(line,httpMsg)!=NULL||strstr(line,sslMsg)!=NULL)
    {
	  ...
	  len=strlen(line)-strlen(spos);
	  strncpy(ip,line,len);
	  ip[len]='\0';
	  //printf("line=%s\n pos=%s\n spos=%s\n ip =%s \n msg=%s\n",line,pos,spos,ip,msg);
	  printf("ip=%s\n",ip);
	  if(cnt==0)
	  {
        strcpy(proto[cnt++],ip);
 ...
 // strcpy(file,msg);
  ...
  for(i=0;i<cnt;i++)
  {
	...
	if(flag==0)
	{
	  ...
	  strcat(proto[i],"\n");
	  fwrite(proto[i], strlen(proto[i]), 1, fp2);
	}
  }
  fclose(fp2);
}
...
int orginal_main(int argc, char **argv) {
  ...
  int main(int argc, char **argv) {
	...
	if((results_file = fopen(results_path, "r")) == NULL)
	{
      return 0;
	}
	else
